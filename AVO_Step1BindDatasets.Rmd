...
pondflux_daily <- pondflux_15min %>%
  group_by(Date) %>% 
  summarize(WSEmin = min(WSE), WSEmean = mean(WSE),WSEmax = max(WSE),precip_event_mm = sum(precip_event_mm), precip_acc_mm = max(precip_acc_mm), VOLmin =min(VOLacft), VOLmean = mean(VOLacft), VOLmax = max(VOLacft), SAmin = min(SAac), SAmean = mean(SAac), SAmax = max(SAac)) %>% 
...
march.storm <- filter(pondflux_15min, Date >= "2018-03-20" & Date <="2018-03-22") %>% 
  mutate(Event_Acc = cumsum(precip_event_mm), VOLgain = VOLacft-8.571884)

april.storm <- filter(pondflux_15min, Date >= "2018-04-06" & Date <="2018-04-07") %>% 
  mutate(Event_Acc = cumsum(precip_event_mm), VOLgain = VOLacft-20.18068)
